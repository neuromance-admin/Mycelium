...
# SporeUpgrade-v0.5.1c

> Vault upgrade file. Brings an existing Mycelium vault from any prior Spore version to v0.5.1c.
> Handles ID migration, persona migration, runtime replacement, and structural updates.
> Does NOT handle first-time setup — that's `_MyceliumInstaller-v0.5.1c.md`.
> When upgrade is complete, this file self-destructs.

---

## What's New in v0.5.1c

- **Handshake Boot Splash**: A terminal-style ASCII banner now renders at the start of every handshake, above the version line. Cosmetic only — no behavioural changes. The version number is tucked into the bottom-right of the banner so the splash and version always travel together.
...
- **Persona Directory Verification**: After reading VaultIdentity, the runtime verifies that `personaDir` resolves correctly before loading persona files. If the directory cannot be confirmed, the session STOPS and surfaces the exact failed path to the owner. Silent persona load failures are no longer possible.
- **Resolution Convention**: `personaDir` relative paths resolve from the vault root file location, not the shell working directory.
...
| State | Action |
|---|---|
| No `VaultIdentity.md` found | WRONG FILE — direct owner to `_MyceliumInstaller-v0.5.1c.md` |
| `sporeVersion` is `0.5.1c` | Health Check only — run Tool 4 from the toolset |
| `sporeVersion` is `0.5.1b` | **Cosmetic upgrade path** — rewrite runtime + toolset, bump version. No ID or persona migration needed. |
| `sporeVersion` older than `0.5.1b` | **Full upgrade path** — may require ID migration, persona migration, structural updates |

Surface detected state to owner before proceeding.

> *"I can see this vault is running Spore-vX.X.X. I'll upgrade it to v0.5.1c — [one-line description of what will change based on current version]. Nothing will be deleted without your confirmation. Shall I proceed?"*

Wait for explicit confirmation.

---

## Cosmetic Upgrade Path (v0.5.1b → v0.5.1c)

If `sporeVersion` is `0.5.1b`, the upgrade is purely cosmetic. No ID migration, no persona migration, no structural changes. Skip directly to Step 5 (propose plan) and Step 6 (execute), with only these actions:

1. Write `_Spore-v0.5.1c.md` to vault root (from Runtime Payload below)
2. Write `_Spore-VMD-Toolset.v0.5.1c.md` to vault root (from Toolset Payload below)
3. Update `System/VaultIdentity.md`:
   - Bump `sporeVersion` to `0.5.1c`
   - Set `upgradedAt: [today] (v0.5.1c)`
4. Update `System/VMD-Index.json`:
   - Bump `sporeVersion`
   - Update `lastUpdated`
5. Archive old root files: `_Spore-v0.5.1b.md`, `_Spore-VMD-Toolset.v0.5.1b.md` (with owner approval)
6. Run Tool 4 — Vault Health Check
...
Compare current vault against v0.5.1c expectations:
...
**Runtime & Toolset**
- Is `_Spore-v0.5.1c.md` present at vault root? If not — needs writing.
- Is `_Spore-VMD-Toolset.v0.5.1c.md` present at vault root? If not — needs writing.
...
> *"Here's the full upgrade plan for v0.5.1c: [list]. Nothing will be deleted without your confirmation. Shall I proceed?"*
...
9. Archive old root files if owner approves (including `_Spore-v0.5.1b.md` and `_Spore-VMD-Toolset.v0.5.1b.md` if present)

Read back every file after writing. Stop and surface if any read-back fails.

### Step 7 — Run Health Check

After upgrade, run Tool 4 — Vault Health Check from `_Spore-VMD-Toolset.v0.5.1c.md`. The toolset was written in Step 6 and is now available at vault root.

### Step 8 — Completion Message

**Cosmetic upgrade (from v0.5.1b):**
> *"Upgrade complete. Vault is now running Spore-v0.5.1c. You'll now see the Mycelium ASCII banner at the start of every handshake. No behavioural changes. Start your next session by pasting `_Spore-v0.5.1c.md` as usual."*

**Full upgrade (from older):**
> *"Upgrade complete. Vault is now running Spore-v0.5.1c. [List of changes applied]. Start your next session by pasting `_Spore-v0.5.1c.md` as usual."*
...
> Write this exactly to vault root as `_Spore-v0.5.1c.md` during upgrade.
...
> Written by `_MyceliumInstaller-v0.5.1c.md` on first install or `_SporeUpgrade-v0.5.1c.md` on upgrade.
> Extended procedures and tools live in `_Spore-VMD-Toolset.v0.5.1c.md`. Consult on demand.
...
**This vault is running Spore-v0.5.1c.**
...
| State | Action |
|---|---|
| VaultIdentity exists, version is `0.5.1c` | Normal session load |
| VaultIdentity exists, version is older | CRITICAL ERROR — run `_SporeUpgrade-v0.5.1c.md` |
| No VaultIdentity | CRITICAL ERROR — run `_MyceliumInstaller-v0.5.1c.md` |
...
> *"VMD loaded. Running Spore-v0.5.1c. Last session: [date] — [one line summary]. Ready to continue. What are we building today?"*
...
> Write this exactly to vault root as `_Spore-VMD-Toolset.v0.5.1c.md` during upgrade.
> Do not modify the payload. Copy exactly.
...
### When to Use
Bringing a vault from an older Spore version to the current one. In most cases, run `_SporeUpgrade-v0.5.1c.md` instead.

### Migration is handled by the Upgrade File
Direct the owner to `_SporeUpgrade-v0.5.1c.md`. Do not attempt manual migration mid-session.
